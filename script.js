...
// Dropdown toggle navigation clicks across desktop and mobile layout rules
...
// Close active menus when clicking away from navigation components
...
async function loadAllAnime() {
    ...
    if (pageUrlParams.get('genre') || pageUrlParams.get('type') || pageUrlParams.get('search')) return;

    try {
        const response = await fetch("https://graphql.anilist.co", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ query: allAnimeQuery })
        });
        ...
        results.forEach(anime => {
            container.insertAdjacentHTML("beforeend", generateCardHtml(anime));
        });
    } catch (err) {
        container.innerHTML = `<p style="color: red;">Failed to load index: ${err.message}</p>`;
    }
}
...
// =================================================================
// SAFE TUTORIAL INJECTION SYSTEM (Does not affect layout or HTML files)
// =================================================================
function injectSafeTutorialModal() {
    if (localStorage.getItem("mirai_seen_guide_v2")) return; // Stop if seen

    // Inject Styles directly into document head cleanly
    ...
    // Create Modal HTML Framework on the fly
    ...
    // Fade animation triggers smoothly after injection
    setTimeout(() => {
        overlay.style.opacity = "1";
        overlay.querySelector(".m-modal-card").style.transform = "translateY(0)";
    }, 800);

    // Cleanup and store memory sequence
    const closeGuide = () => {
...
// Global Document Initializer Loop
document.addEventListener("DOMContentLoaded", () => {
    // Inject the popup safely
    ...
    if (allAnimeContainer) {
        ...
        const pageUrlParams = new URLSearchParams(window.location.search);
        const requestedGenre = pageUrlParams.get('genre');
        const requestedType = pageUrlParams.get('type');
        const requestedSearch = pageUrlParams.get('search'); 
        ...
        if (titleElement) {
            if (requestedSearch) {
                ...
                titleElement.innerHTML = `<i class="fas fa-th-list"></i> ${requestedGenre} Anime`;
            } else if (requestedType) {
                const formatNames = { "TV": "TV Series", "MOVIE": "Movies", "OVA": "OVAs", "ONA": "ONAs", "SPECIAL": "Specials", "MUSIC": "Music" };
                titleElement.innerHTML = `<i class="fas fa-th-list"></i> ${formatNames[requestedType] || requestedType}`;
            }
        }
        ...
        async function loadAllAnimePaginated(page) {
            ...
            const query = `
            query ($page: Int, $genre: String, $format: MediaFormat, $search: String) { 
                Page (page: $page, perPage: 24) { 
                    media (type: ANIME, sort: POPULARITY_DESC, genre: $genre, format: $format, search: $search) { 
                        id 
                        title { english romaji } 
                        coverImage { large } 
                        episodes 
